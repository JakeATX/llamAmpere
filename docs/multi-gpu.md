...
| Abort *"GGML_ASSERT(tensor) failed"* in `ggml_backend_tensor_set` on the first request with `--spec-type draft-mtp` | MTP drafting is not yet supported together with `--split-mode tensor` (any weight type; the draft context's graph inputs are not registered with the meta buffer). Run without the MTP drafter or use `--split-mode layer`. |
