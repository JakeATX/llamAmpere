...
| Log line *"MTP context will share the target context's compute buffers"* is missing with `--spec-type draft-mtp` | Expected with `--split-mode tensor`: the MTP draft context gets its own compute buffers there (the meta backend's per-device tensor mappings do not survive the target's graph allocations), so the drafter costs one extra compute buffer per device. |
