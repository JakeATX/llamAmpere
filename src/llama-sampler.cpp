...
struct llama_sampler_dist : public llama_sampler_backend {
    ...
    // input of the current sampling graph: [1, n_rows], one uniform sample per output row
    ggml_tensor * inp_uniform;
    int64_t       n_rows = 1;

    void copy_state(const llama_sampler_dist & src) {
        // note: inp_uniform and backend_transactional belong to the current sampling graph
